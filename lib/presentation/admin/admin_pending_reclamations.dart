...
class AdminPendingReclamations extends StatefulWidget {
  const AdminPendingReclamations({Key? key}) : super(key: key);

  @override
  State<AdminPendingReclamations> createState() => _AdminPendingReclamationsState();
}

class _AdminPendingReclamationsState extends State<AdminPendingReclamations> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: StreamBuilder(
          stream: FirebaseFirestore.instance.collection('reclamations').where("status", isEqualTo: 0).snapshots(),
                                      ...
                                      SlidableAction(
                                        onPressed: (BuildContext ctx) {
                                          ...
                                          notifDoc.set({
                                            ...
                                            "content": "Un admin a traité votre reclamation",
                                            "date": DateTime.now(),
                                            "id": notifDoc.id
                                          });
                                          Fluttertoast.showToast(
                                            msg: "Reclamation traité avec succès",
